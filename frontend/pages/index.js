...
import { Phone, Email, WorkOutline, CheckCircle, Code, KeyboardArrowUp } from '@mui/icons-material';
...
export default function Home() {
  ...
  const [loadingResumeName, setLoadingResumeName] = useState(false);
  ...
  const fetchResumeName = async (resumeText) => {
    try {
      setLoadingResumeName(true);
      const response = await fetch('http://127.0.0.1:8000/getNames', {
        method: 'POST',
        headers: {
      ...
      const data = await response.json();
      setResumeName(data.name || 'Unknown');
      console.log('Resume Name API Response:', data.name);
    } catch (error) {
      console.error('Error fetching resume name:', error);
      setResumeName('Unknown');
    } finally {
      setLoadingResumeName(false);
    }
  };
  ...
  const handleSubmitInsights = async () => {
    ...
    if (resumeFile) {
      ...
      // Word document handling (.docx)
        ...
        fileReader.onload = async function () {
          try {
            ...
            alert('Failed to extract text from the Word document. Please try another file.');
          }
        };
        fileReader.readAsArrayBuffer(resumeFile);
      }
      // Word document handling (.doc)
      else if (fileType === 'application/msword' || fileExtension === 'doc') {
        alert('Legacy .doc files are not fully supported. For best results, please save as .docx and try again.');
        // Basic attempt to read .doc files, but may not work reliably
        const fileReader = new FileReader();
        fileReader.onload = async function () {
          try {
            // Simple text extraction - may not work well for all .doc files
            ...
            console.error('Error extracting text from legacy Word document:', error);
            alert('Failed to extract text from the Word document. Please convert to .docx and try again.');
          }
        };
      ...
      // Plain text files
        ...
        alert('Unsupported file type. Please upload a PDF, DOCX, DOC, or TXT file.');
      }
    }
  };
